# CityRide — Data Ingestion & Pipeline
...
### 6.4 Rentals (with Anomaly Flags)
...
## 7. MERGE: VALIDATED → CURATED

### 7.1 dim_stations (SCD Type-2)
...
### 7.4 fact_rentals (MERGE + Derived Fields)
...
task_raw_to_val_bikes ─────┤──▶ task_curate_dimensions ──┐
task_raw_to_val_users ─────┘       (SCD2 upserts)        │──▶ task_curate_rentals
task_raw_to_val_rentals ───────────────────────────────────┘      (fact MERGE)
      (all: root, scheduled 5 min)
```
